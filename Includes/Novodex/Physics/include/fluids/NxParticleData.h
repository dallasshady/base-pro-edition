...
class NxParticleData
	...
	*/
	...
	/**
	Not available in the current release.
	Same as bufferPos but for particle lifetimes. A particle lifetime consists of one 32 bit 
	...
	*/
	...
	/**
	Not available in the current release.
	Same as bufferDensity but for particle densities. A particle density consists of one 32 bit 
	float. If set to NULL, densities are not read or written to.
	*/
	NxF32*					bufferDensity;
...
NX_INLINE bool NxParticleData::isValid() const
	{
	if (numParticlesPtr && !(bufferPos || bufferVel || bufferLife || bufferDensity || bufferFlags)) return false;
	if ((bufferPos || bufferVel || bufferLife || bufferDensity || bufferFlags) && !numParticlesPtr) return false;
