...
void Jumper::Tracking::machine() {
	...
	// CONDITIONS
	// torque
	const PxTransform freeFallPose = _jumper->getFreefallActor()->getGlobalPose();
	const PxVec3 spin_axis_x = freeFallPose.q.getBasisVector0();
	const PxVec3 spin_axis_z = freeFallPose.q.getBasisVector2();
    ...
    PxVec3 y = globalPose.q.getBasisVector2();
    PxVec3 z = -globalPose.q.getBasisVector1();
	...
	if (this->machine_state() == MACHINE_STATIC_ARCH) {
		...
		if (actions[MACHINE_ACTION_TORQUE_FRONT]) {
			_phActor->addTorque(PxVec3(130.0f, 0.0f, 0.0f));
		}
	...
	// TEMP FRONT FLIP
		...
		if (actions[MACHINE_ACTION_TRACK] || SPIN_X < spin_for_fb_flip && !actions[MACHINE_ACTION_TORQUE_FRONT]) {
			...
			if (actions[MACHINE_ACTION_TORQUE_FRONT]) {
				...
				_phActor->addTorque(PxVec3(torque, 0.0f, 0.0f));
			}
		}
	}
