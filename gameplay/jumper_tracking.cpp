...
Jumper::Tracking::Tracking(Jumper* jumper, PxRigidDynamic* phActor, MatrixConversion* mc) :
    JumperAction( jumper )
{
	...
	//PxVec3 motionDir = _phActor->getLinearVelocity();
	//motionDir.normalize();
	//PxVec3 canopyDown  = _phActor->getGlobalPose().M.getColumn(2);
...
void Jumper::Tracking::updateProceduralAnimation(float dt)
{
	...
	// add wind
	//PxVec3 wind = _jumper->getScene()->getWindAtPoint( _phActor->getGlobalPose().t );
	//velocity += wind;
...
void Jumper::Tracking::update(float dt)
{
	...
	//PxMat44 pxmat = PxMat44(_phActor->getGlobalPose());
	//Matrix4f mat = Matrix4f(pxmat.column0[0], pxmat.column0[1], pxmat.column0[2], pxmat.column0[3],
	//						pxmat.column1[0], pxmat.column1[1], pxmat.column1[2], pxmat.column1[3],
	//						pxmat.column2[0], pxmat.column2[1], pxmat.column2[2], pxmat.column2[3],
	//						pxmat.column3[0], pxmat.column3[1], pxmat.column3[2], pxmat.column3[3]);
	//Matrix4f mat2 = wrap(_phActor->getGlobalPose());

    // synchronize physics & render
	//PxTransform pose = _phActor->getGlobalPose();
	//pose.p *= 100.0f;

	//Vector3f pos = wrap(_phActor->getGlobalPose().p);

		//_clump->getFrame()->setPos(pos);
		//_clump->getFrame()->setMatrix(mat);
		//_clump->getFrame()->setMatrix(wrap(_phActor->getGlobalPose()));
			_phActor->addTorque(PxVec3(100000000, 100000000, 100000000), PxForceMode::eIMPULSE);
			_phActor->setAngularDamping(0.05f);


			PxMat44 pxmat = PxMat44(_phActor->getGlobalPose());
			
			PxTransform result;
			PxVec3  x( pxmat.column0[0], pxmat.column1[0], pxmat.column2[0] );
			PxVec3  y( pxmat.column0[1], pxmat.column1[1], pxmat.column2[1] );
			PxVec3  z( pxmat.column0[2], pxmat.column1[2], pxmat.column2[2] );
			x.normalize();
			y.normalize();
			z.normalize();
			result.q = PxQuat(PxMat33( x,y,z ));
			//result.q = PxQuat(PxIDENTITY::PxIdentity);
			result.p = PxVec3( pxmat.column3[0], pxmat.column3[1], pxmat.column3[2] );
...
*/
	...
	_clump->getFrame()->setMatrix( _matrixConversion->convert( wrap(result) ) ); 
...
void Jumper::Tracking::updatePhysics(void)
{
	//PxTransform pose = _phActor->getGlobalPose();
	//if (pose.p.y < 300000) {
	//	_phActor->setGlobalPose(PxTransform(0, 400000, 0, PxQuat(PxIDENTITY::PxIdentity)));
	//}
	...
	//if (spine->up) {
	//	_phActor->addForce(PxVec3(0, 1000.0f, 0));
	//}
	/// PHYSICS
	// altitude [m]
	PxVec3 pos = _phActor->getGlobalPose().p;
	...
	if (!currentPose.torque.isZero()) {
		_phActor->addTorque(currentPose.torque);
	}
	// forces
	if (!currentPose.force.isZero()) {
		PxRigidBodyExt::addLocalForceAtLocalPos(*_phActor, currentPose.force, PxVec3(0,0,0));
	}
	...
	//		_phActor->addLocalTorque(PxVec3(0.0f, 0.0f, -900.0f));
	//	} else {
	//		_phActor->addLocalTorque(PxVec3(0.0f, 900.0f, 0.0f));
	//	}
	...
	//	_phActor->addLocalTorque(PxVec3(turnx, 0.0f, 0.0f));
	...
	// add wind
	const PxVec3 wind = _jumper->getScene()->getWindAtPoint( _phActor->getGlobalPose().p );
	velocity += wind;
	...
	PxVec3 jumperAt = wrap(_jumper->getClump()->getFrame()->getAt());
	PxVec3 jumperUp = wrap(_jumper->getClump()->getFrame()->getUp());

	PxTransform globalPose = _phActor->getGlobalPose();
	PxVec3 x = globalPose.q.getBasisVector0();
    PxVec3 y = globalPose.q.getBasisVector1();
    PxVec3 z = -globalPose.q.getBasisVector2();
	...
	for (int i = 0; i < POSEANIM_JOINTS; ++i) {
		...
		Fd_total += Fd;
		PxVec3 Drag = Fd*VecFd * (1.0f - this->currentPose.L_now) * Dfrac;

		// torso position is middle between all other airfoils
		//if (i == POSEANIM_JOINT_TORSO) {
		//	PxVec3 torso = PxVec3(0,0,0);
		...
		//} else {
		PxRigidBodyExt::addForceAtLocalPos(*_phActor, Drag, this->currentPose.airfoils_now[i]);
		//PxRigidBodyExt::addLocalForceAtLocalPos(*_phActor, Drag, this->currentPose.airfoils_now[i], PxForceMode::eIMPULSE);
		//}
	}
	
	// convert some drag to lift
	// reaction
	PxVec3 motionDir = velocity;
	motionDir.normalize();
	PxVec3 canopyDown  = _phActor->getGlobalPose().q.getBasisVector2();
	canopyDown.normalize();
	float reaction = canopyDown.dot( motionDir );
	...
	//_phActor->addLocalForce(PxVec3(0,0,1) * Fd_total * Lfrac);

	PxVec3 Flift = PxVec3(0,0,0);
	if (reaction != 0.0f) {
		//Flift = PxVec3(0,1,0) * Fd_total * reaction * this->currentPose.L_now * 1.8f;
		
		Flift = PxVec3(0,0,1) * Fd_total * Lfrac * (1.0f + this->currentPose.L_now);
		if (this->useWingsuit) {
			Flift += PxVec3(0,1,0) * Fd_total * Lfrac * 0.7f;
			Flift += PxVec3(0,0,1) * Fd_total * Lfrac * (1.0f + this->currentPose.L_now) * 0.13f;
		}
		// add lift
		PxRigidBodyExt::addLocalForceAtPos(*_phActor, Flift, PxVec3(PxZERO::PxZero));
	}
