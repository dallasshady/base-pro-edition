...
Airfoil::Airfoil(NxActor *phActor) {
	Airfoil();
	_phActor = phActor;
}
Airfoil::Airfoil(NxActor *phActor, float Aw, float Cd, NxVec3 pos) {
	_Aw = Aw;
	_Cd = Cd;
	_Pa = -1.0f;
	_pos = pos;
...
void Airfoil::setPosition(NxVec3 pos) {
	_pos = pos;
}
...
void Airfoil::calcForce(NxVec3 dir, float Pa) {
	return;
	if (!_phActor) return;
	...
	NxVec3 pointVel = _phActor->getLinearVelocity();
	...
	pointVel.normalize();
	NxVec3 Drag = Fd * -pointVel;
	
	NxVec3 pointVel90CCWx = NxVec3(pointVel.x, pointVel.z, -pointVel.y);
	...
	//getCore()->logMessage("");
	_phActor->addForceAtLocalPos(Drag, _pos);
	_phActor->addLocalForceAtLocalPos(Lift, _pos);
}
...
float Airfoil::AOA(NxVec3 &airspeed) {
    ...
    NxVec3 y = freeFallPose.M.getColumn(2);
    NxVec3 z = -freeFallPose.M.getColumn(1);

    //NxVec3 hz = z; hz.y = 0; hz.normalize();
	NxVec3 velocity = airspeed;
	velocity.normalize();
