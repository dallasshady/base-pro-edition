...
void Jumper::Flip::updatePhysics(void)
{
    ...
    // control torque
    NxVec3 Tctrl( 0,0,0 );
    Tctrl += NxVec3(1,0,0) * -_jumper->getVirtues()->getSteerPitch() * _jumper->getSpinalCord()->down * 10.0f;
    Tctrl += NxVec3(1,0,0) * _jumper->getVirtues()->getSteerPitch() * _jumper->getSpinalCord()->up * 10.0f;
    Tctrl *= It;
