...
public:
	...
	NxVec3 airfoils[POSEANIM_JOINTS];			// CHANGE in airfoil (drag element) positions [m]
	float airfoils_area[POSEANIM_JOINTS];		// CHANGE in airfoil (drag element) area [m^2]
	NxVec3 jointaxis[POSEANIM_JOINTS_ALL];		// CHANGE in mesh joint rotation axes
	float  jointangles[POSEANIM_JOINTS_ALL];	// CHANGE in mesh joint rotation angles

	NxVec3 torque;				// added torque
	NxVec3 force;				// added force
...
public:
	...
	NxVec3 airfoils[POSEANIM_JOINTS];			// airfoil (drag element) positions at rest [m]
	float airfoils_area[POSEANIM_JOINTS];		// airfoil (drag element) area at rest [m^2]
	NxVec3 jointaxis[POSEANIM_JOINTS_ALL];		// mesh joint rotation axes
	float  jointangles[POSEANIM_JOINTS_ALL];	// mesh joint rotation angles

	NxVec3 airfoils_now[POSEANIM_JOINTS];		// airfoil (drag element) positions based on current animation [m]
